...
import { WebhookScreen } from "./WebhookScreen";
...
export const MainScreen: React.FC<MainScreenProps> = ({ onOpenOptions }) => {
  ...
  const handleConnectWallet = async () => {
    ...
    try {
      ...
      if (
        infoResponse.type === "METAMASK_INFO" &&
        infoResponse.data?.selectedAddress
      ) {
      ...
      if (err.message?.includes("already pending")) {
        setError(
          "Connection request is already pending. Please check MetaMask popup."
        );
      } else {
  ...
  // Check webhook enabled status
  useEffect(() => {
    const checkWebhookStatus = async () => {
      const result = await chrome.storage.local.get(["webhookEnabled"]);
      setIsWebhookEnabled(result.webhookEnabled || false);
    };
    checkWebhookStatus();
    ...
    const handleStorageChange = (changes: {
      ...
      if (changes.webhookEnabled) {
        setIsWebhookEnabled(changes.webhookEnabled.newValue || false);
      }
    };
  ...
  if (showWebhookScreen) {
    return <WebhookScreen onBack={() => setShowWebhookScreen(false)} />;
  }
    ...
    <div style={styles.container}>
      <div style={styles.header}>
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          {isWebhookEnabled && (
            <button
              onClick={() => setShowWebhookScreen(true)}
      ...
      <div style={styles.accountContainer}>
        ...
        {isMetaMaskInstalled === false ? (
          ...
          <>
            <div style={styles.accountAddress}>{formatAddress(account)}</div>
            <button
